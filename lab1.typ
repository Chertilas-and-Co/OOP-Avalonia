...
Научиться пользоваться простейшими компонентами организации переключений (`RadioButton`). Написать и отладить программу разветвляющегося алгоритма.
...
Изучить простейшие средства отладки программ в среде Visual Studio. Составить и отладить программу циклического алгоритма.
...
Если результат выражения `B` окажется ложным при первой проверке, то тело цикла не выполнится ни разу. Отметим, что если условие `B` во время работы цикла не будет изменяться, это может привести к возникновению бесконечного цикла, то есть невозможности выхода из него. Поэтому внутри тела должны находиться операторы, приводящие к изменению значения выражения `B` так, чтобы цикл мог корректно завершиться.
...
Сначала выполняется оператор `S`, а затем анализируется значение выражения `B`: если оно истинно, то управление передается оператору `S`, если ложно --- цикл завершается, и управление передается на оператор, следующий за условием `B`. Так как условие В проверяется после выполнения тела цикла, то в любом случае тело цикла выполнится хотя бы один раз.
...
Инициализация используется для объявления и/или присвоения начальных значений величинам, используемым в цикле в качестве параметров (счётчиков). В этой части можно записать несколько операторов, разделённых запятой. Областью видимости переменных, объявленных в части инициализации цикла, является цикл и вложенные в него блоки. Инициализация выполняется только один раз в начале исполнения цикла.

Выражение определяет условие выполнения цикла: если его результат истинен --- цикл выполняется. Истинность выражения проверяется перед каждым выполнением тела цикла, таким образом, цикл с параметром реализован как цикл с предусловием. В блоке выражения через запятую можно записать несколько логических выражений, тогда запятая равносильна операции логическое И (`&&`).

Модификация выполняется после каждой итерации цикла и служит обычно для изменения параметров цикла. В части модификации можно также записать несколько операторов через запятую.
...
// Скриншот со списком ошибок компиляции
...
// Скриншот с фрагментом кода с breakpoint
