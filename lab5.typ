...
1. Avalonia.Ski. Осторожно! Так как может быть конфликт с версией самой Avalonia
...
3. SkiaSharp.NativeAssets.Linux. То, что уникально для каждой платформы
`dotnet add package SkiaSharp.NativeAssets.Linux` 

Чтобы проверить, что оно добавилось в проект, убедитесь, что в .csproj есть следующие строки
```xaml
...
В System.Windows.Forms предусмотрен удобный объектно-ориентированный способ, позволяющий приложению при необходимости перерисовывать окно формы в любой момент времени. Когда вся клиентская область окна формы или часть этой области требует перерисовки, форме передается событие `Paint`. В Avalonia с использованием SkiaSharp это событие заменяется `Render` методом кастомного контроля или `RenderSkia` событием для специализированных компонентов.

Также в Windows существует GDI (Graphics Device Interface) --- интерфейс , предназначенным для представления графических объектов. Объект Graphics представляет поверхность рисования GDI и используется для создания графических изображений. В силу особенностей работы графики создать экземпляр класса Graphics нельзя, поэтому для обхода зачастую используют ссылку на объект для рисования, полученную из параметра `PaintEventArgs`, который передаётся в обработчик события `Paint`.
...
В Windows Forms используется PictureBox для отображения Bitmap. В Avalonia для аналогичных целей используется Image контроль, но его нельзя напрямую присвоить SKBitmap, поэтому требуется конвертирование SKBitmap в Avalonia Bitmap.
...
3) Создайте функцию, отрисовывающую окружность. Центр окружности совпадает с центром изображения. Все точки вне окружности закрашиваются черным цветом. Все точки внутри окружности остаются неизменными. Радиус окружности задается пользователем.
