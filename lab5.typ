...
В Avalonia для создания графического редактора используется связка из нескольких компонентов, каждый из которых выполняет свою уникальную функцию в цепочке «отрисовка → отображение». Основным визуальным элементом выступает Canvas, а результат рисования накладывается на него как фон через специальную кисть.
Ключевые компоненты:
